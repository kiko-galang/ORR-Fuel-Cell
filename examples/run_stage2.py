...
def _plot_comparison(vs1, sols1, vs2, sols2, mesh, p):
    ...
    from book_style import FIGSIZE_LARGE, BOOK_COLORS, add_panel_labels, savefig_book
    ...
    axes[0].plot(J1, vs1, marker="o", color=BLACK, label="Stage 1 (no-flux)")
    axes[0].plot(J2, vs2, marker="s", ls="--", color=BLUE, label="Stage 2 (Sherwood)")
    ...
    axes[1].plot(xc, np.exp(ln1), color=BLACK, label=f"Stage 1  V={vs1[-1]:.3f}")
    axes[1].plot(xc, np.exp(ln2), ls="--", color=BLUE, label=f"Stage 2  V={vs2[-1]:.3f}")
    ...
    fig.tight_layout()
    savefig_book(fig, "stage2_comparison.png")
    plt.close()
